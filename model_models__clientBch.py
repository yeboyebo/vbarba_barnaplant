...
# @class_declaration vbarba_barnaplant #
class vbarba_barnaplant(interna):

    def vbarba_barnaplant_formRecordlineaspedidoscli(self, fN, dict, prefix, pk):
        print("????", fN)
        if fN == "referencia":
            querydict = {}
            querydict["p_l"] = 50
    ...
    def __init__(self, context=None):
        super(vbarba_barnaplant, self).__init__(context)

    def formRecordlineaspedidoscli(self, fN, dict, prefix, pk):
        return self.vbarba_barnaplant_formRecordlineaspedidoscli(fN, dict, prefix, pk)


# @class_declaration head #
class head(vbarba_barnaplant):
